...
public class UserControllerTest {
    ...
    void testFindUserByIdSuccess() throws Exception {
        ...
        // given
        given(this.userService.findById(2))
            .willReturn(this.users.get(1));
    ...
    void testDeleteUserSuccess() throws Exception {
        ...
        // when and run
        this.mockMvc.perform(delete(this.baseUrl + "/users/2").contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.flag").value(true))
            .andExpect(jsonPath("$.code").value(StatusCode.SUCCESS))
    ...
    void testDeleteUserWithNonExistentId() throws Exception {
        ...
        // when and run
        this.mockMvc.perform(delete(this.baseUrl + "/users/5").contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.flag").value(false))
            .andExpect(jsonPath("$.code").value(StatusCode.NOT_FOUND))
            .andExpect(jsonPath("$.message").value("Could not find user with Id 5"))
            .andExpect(jsonPath("$.data").isEmpty());
